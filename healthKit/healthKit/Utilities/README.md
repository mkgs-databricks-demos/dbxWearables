...
| `KeychainHelper.swift` | Multi-account Keychain wrapper (service: `com.dbxwearables.api`) used to store the Databricks SPN client ID/secret and cached OAuth access tokens. Items use `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly` so background HealthKit sync can read them post-reboot but they never iCloud-sync. |
